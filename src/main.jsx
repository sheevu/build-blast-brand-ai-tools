...
// --- Sudarshan AI Labs Landing Page (from App.jsx) ---
...
function Services() {
  // This section remains as a high-level overview
  return (
...
function PricingSection() {
  ...
  // Filtered list for 'bundle' and 'individual' tabs
  ...
  return (
    <section id="pricing" className="mt-16 md:mt-24">
      ...
      <div className="mt-8">
        {active === 'social' ? (
          // If 'Social Bundles' is clicked, show the new comparison table
        ...
        <div className="grid grid-cols-4 gap-px bg-white/10 p-px rounded-b-2xl">
          {data.map((item, itemIndex) => (
            <React.Fragment key={item.feature}>
              ...
              <div className={`bg-[#050814] p-4 text-center ${itemIndex === data.length - 1 ? 'border-b border-transparent' : ''}`}>
                {renderCell(item.foundation)}
              </div>
              <div className={`bg-gradient-to-b from-[#003428]/20 to-[#050814] p-4 text-center ${itemIndex === data.length - 1 ? 'border-b border-transparent' : ''}`}>
                {renderCell(item.expansion)}
              </div>
              <div className={`bg-[#050814] p-4 text-center ${itemIndex === data.length - 1 ? 'rounded-br-2xl' : ''}`}>
         ...
         {/* --- Footer Button Row --- */}
        <div className="grid grid-cols-4 gap-px bg-white/10 p-px rounded-b-2xl mt-[-10px]">
          <div className="bg-[#0B0F19] p-4 rounded-bl-2xl"></div>
           {pricingPlans.filter(p => p.category === 'social').map((plan, index) => (
            ...
            >
               <button className="w-full rounded-full bg-gradient-to-r from-[#00F1A0] to-[#00FFFF] py-2 text-black font-semibold text-sm">Book Now</button>
             </div>
           ))}
        </div>
      </div>
...
// --- React DOM Render (from main.jsx) ---
